...
def oneStep(grove: list[list[str]], functions):
    ...
    return res, grove, functions

def cutOffSides(grove: list[list[str]]) -> list[list[str]]:
    coords = []
    for i, _ in enumerate(grove):
        for j, _ in enumerate(grove[i]):
            if grove[i][j] == '#':
                coords.append((i, j))
...
grove = extend(grove, 2000)
round = 0
keepGoing = True
while keepGoing:
    keepGoing, grove, functions = oneStep(grove, functions)
    round += 1
print(round)
